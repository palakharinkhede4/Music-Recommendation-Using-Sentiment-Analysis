import { Track, EmotionType } from '../types';

const API_BASE = 'http://127.0.0.1:8000/api';

const MOCK_TRACKS: Track[] = [
  // 1. HAPPY (Dance & Upbeat Music)
  {
    id: "hp_01",
    title: "Levitating",
    artist: "Dua Lipa ft. DaBaby",
    album: "Future Nostalgia",
    mood: "Happy",
    genre: "Dance / Upbeat Pop",
    duration: 203,
    audioUrl: "/audio/happy/Dua Lipa - Levitating Featuring DaBaby (Official Music Video).mp3",
    coverUrl: "https://images.unsplash.com/photo-1514525253161-7a46d19cd819?w=500&auto=format&fit=crop&q=80",
    themeColor: "#EAB308",
    bpm: 124
  },
  {
    id: "hp_02",
    title: "Heat Waves",
    artist: "Glass Animals",
    album: "Dreamland",
    mood: "Happy",
    genre: "Indie Pop / Dance",
    duration: 238,
    audioUrl: "/audio/happy/Glass Animals - Heat Waves (Official Video).mp3",
    coverUrl: "https://images.unsplash.com/photo-1470225620780-dba8ba36b745?w=500&auto=format&fit=crop&q=80",
    themeColor: "#F59E0B",
    bpm: 120
  },
  {
    id: "hp_03",
    title: "Senorita",
    artist: "Farhan Akhtar, Hrithik Roshan, Abhay Deol",
    album: "Zindagi Na Milegi Dobara",
    mood: "Happy",
    genre: "Upbeat Flamenco Pop",
    duration: 231,
    audioUrl: "/audio/happy/Senorita Zindagi Na Milegi Dobara Full HD Video Song Farhan Akhtar, Hrithik Roshan, Abhay Deol.mp3",
    coverUrl: "https://images.unsplash.com/photo-1507525428034-b723cf961d3e?w=500&auto=format&fit=crop&q=80",
    themeColor: "#FBBF24",
    bpm: 128
  },

  // 2. SAD (Motivational & Uplifting Songs)
  {
    id: "sd_01",
    title: "Kar Har Maidaan Fateh",
    artist: "Sukhwinder Singh, Shreya Ghoshal",
    album: "Sanju",
    mood: "Sad",
    genre: "Motivational Anthem",
    duration: 311,
    audioUrl: "/audio/sad/Sanju KAR HAR MAIDAAN FATEH Full Video Song Ranbir Kapoor Rajkumar Hirani.mp3",
    coverUrl: "https://images.unsplash.com/photo-1518837695005-2083093ee35b?w=500&auto=format&fit=crop&q=80",
    themeColor: "#3B82F6",
    bpm: 105
  },
  {
    id: "sd_02",
    title: "Unstoppable",
    artist: "Sia",
    album: "This Is Acting",
    mood: "Sad",
    genre: "Motivational Pop",
    duration: 217,
    audioUrl: "/audio/sad/Sia - Unstoppable (Lyrics).mp3",
    coverUrl: "https://images.unsplash.com/photo-1499346030926-9a72daac6ce6?w=500&auto=format&fit=crop&q=80",
    themeColor: "#60A5FA",
    bpm: 92
  },
  {
    id: "sd_03",
    title: "Hall of Fame",
    artist: "The Script ft. will.i.am",
    album: "#3",
    mood: "Sad",
    genre: "Uplifting Rock Pop",
    duration: 202,
    audioUrl: "/audio/sad/The Script - Hall of Fame (feat. will.i.am).mp3",
    coverUrl: "https://images.unsplash.com/photo-1509114397022-ed747cca3f65?w=500&auto=format&fit=crop&q=80",
    themeColor: "#2563EB",
    bpm: 85
  },

  // 3. NEUTRAL (Ambient Chillout & Lo-Fi)
  {
    id: "nt_01",
    title: "Cozy Retro Lofi Beat",
    artist: "Lofi Chill Producer",
    album: "Retro Sessions",
    mood: "Neutral",
    genre: "Lo-Fi Hip Hop",
    duration: 154,
    audioUrl: "/audio/neutral/Cozy Retro Lofi Beat [Uj4OMZzV7PU].mp3",
    coverUrl: "https://images.unsplash.com/photo-1501386761578-eac5c94b800a?w=500&auto=format&fit=crop&q=80",
    themeColor: "#8B5CF6",
    bpm: 80
  },
  {
    id: "nt_02",
    title: "Luv(sic.) pt3",
    artist: "Nujabes ft. Shing02",
    album: "Modal Soul",
    mood: "Neutral",
    genre: "Jazzhop / Lo-Fi",
    duration: 335,
    audioUrl: "/audio/neutral/Nujabes - Luv(sic.) pt3 (feat. Shing02) Official Audio.mp3",
    coverUrl: "https://images.unsplash.com/photo-1447752875215-b2761acb3c5d?w=500&auto=format&fit=crop&q=80",
    themeColor: "#A78BFA",
    bpm: 88
  },
  {
    id: "nt_03",
    title: "The Last Days of Summer 2019",
    artist: "Chillout Ambient",
    album: "Summer Sunset",
    mood: "Neutral",
    genre: "Ambient Chillout",
    duration: 180,
    audioUrl: "/audio/neutral/The Last days of Summer 2019 [JNZIrlwyke4].mp3",
    coverUrl: "https://images.unsplash.com/photo-1519681393784-d120267933ba?w=500&auto=format&fit=crop&q=80",
    themeColor: "#7C3AED",
    bpm: 84
  },

  // 4. ANGRY (Calm & Relaxing Music)
  {
    id: "ag_01",
    title: "Idea 10",
    artist: "Gibran Alcocer",
    album: "Piano Solitude",
    mood: "Angry",
    genre: "Calm Classical Piano",
    duration: 190,
    audioUrl: "/audio/angry/Idea 10 [NfOg7uNLolA].mp3",
    coverUrl: "https://images.unsplash.com/photo-1507525428034-b723cf961d3e?w=500&auto=format&fit=crop&q=80",
    themeColor: "#EF4444",
    bpm: 60
  },
  {
    id: "ag_02",
    title: "Spirit of Life",
    artist: "Blackmill",
    album: "Miracle",
    mood: "Angry",
    genre: "Melodic Relaxing Ambient",
    duration: 210,
    audioUrl: "/audio/angry/Spirit of Life [vUlpSbdGo9U].mp3",
    coverUrl: "https://images.unsplash.com/photo-1451187580459-43490279c0fa?w=500&auto=format&fit=crop&q=80",
    themeColor: "#DC2626",
    bpm: 65
  },
  {
    id: "ag_03",
    title: "Time For a Coffee",
    artist: "Julian R",
    album: "Peaceful Moments",
    mood: "Angry",
    genre: "Soft Relaxing Guitar",
    duration: 165,
    audioUrl: "/audio/angry/Time For a Coffee [l4_MqCxnH0U].mp3",
    coverUrl: "https://images.unsplash.com/photo-1483664852095-d6cc6870702d?w=500&auto=format&fit=crop&q=80",
    themeColor: "#B91C1C",
    bpm: 58
  }
];

export async function fetchRecommendations(mood: EmotionType): Promise<Track[]> {
  try {
    const res = await fetch(`${API_BASE}/tracks/recommendations?mood=${mood}`);
    if (res.ok) {
      return await res.json();
    }
  } catch (err) {
    console.warn("Backend API offline; using local user audio dataset.", err);
  }

  const matched = MOCK_TRACKS.filter(t => t.mood === mood);
  const rest = MOCK_TRACKS.filter(t => t.mood !== mood);
  return [...matched, ...rest];
}

export async function logSentiment(mood: EmotionType, confidence: number): Promise<void> {
  try {
    await fetch(`${API_BASE}/sentiment/log`, {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({ mood, confidence })
    });
  } catch (e) {
    // Silent fallback
  }
}
